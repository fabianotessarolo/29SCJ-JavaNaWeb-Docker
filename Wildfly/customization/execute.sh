...
data-source add --name=MySqlDS --driver-name=mysql --jndi-name=	java:/MySqlDS --connection-url=jdbc:mysql://$MYSQL_URI/mysqldb?useUnicode=true&amp;characterEncoding=UTF-8 --user-name=root --password=root --use-ccm=false --max-pool-size=25 --blocking-timeout-wait-millis=5000 --enabled=true
...
# Deploy the WAR
cp /opt/jboss/wildfly/customization/Netgifx.war $JBOSS_HOME/$JBOSS_MODE/deployments/Netgifx.war
